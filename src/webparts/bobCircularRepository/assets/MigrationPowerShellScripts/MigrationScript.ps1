...
1. Department Values not matching the department list item
2. Add ListItem
3. Update List Item based on Circular Number
4. Circular Number Exist
5. Logs & Error Logs Creation in csv
6. Permission Break & Everyone is given read access
7. Change Regional Settings to UTC Cordinated Universal Time
#>
<#
...
<# 1.Create Logs Name Folder , 2.Provide Correct Folder Path #>
$circularClassification = "Master";
$masterCircularPath = "C:\Vivek\CRAT Tool\mastercirculars.xlsx";
$circularPath = "C:\Vivek\CRAT Tool\MetaDataCirculars.xlsx";
$OutLogPath = "C:\Vivek\CRAT Tool\Logs\";
$ErrorOutLogPath = "C:\Vivek\CRAT Tool\Logs\";
$siteUrl = "https://mlaitechio.sharepoint.com/sites/BOB_New";
$listName = "CircularRepository";
$FolderPath = "C:\Vivek\CRAT Tool\"
    ...
    ForEach ($record in $masterCircularData) {
        ...
        if ($null -ne $currentItem) {
            ...
            try {
                ...
                $addListItem = Add-PnPListItem -List $listName -Values @{
                    "Subject"            = $($record.'SUBJECT');
                    "MigratedDepartment" = $($record.'DEPTNAME');
                    "Department"         = $($record.'DEPTNAME');
                    "Classification"     = $circularClassification;
                ...
                AddAttachmentsFromFolder -ListItem $addListItem -circularNumber $circularNumber -docPath $record.'DOC PATH' -isAdd $true;
            }
            catch {
                $global:errorLogs += WriteErrorLogs -circularNumber "$($circularNumber)" -message "$($_.Exception.Message)";  
            }
...
Function CircularMigration() {
    ...
    ForEach ($record in $circularData) {
        ...
        # Parse the input date string into a DateTime object 
        $convertedDate = ConvertDate -uploadDate $record.'CIR_DATE';
        ...
        if ($null -ne $currentItem) {
            ...
            try {
                ...
                AddAttachmentsFromFolder -ListItem $listItemUpdate -circularNumber $circularNumber -docPath $record.'DOC_PATH' -isAdd $false;
            ...
            try {
                ...
                AddAttachmentsFromFolder -ListItem $addListItem -circularNumber $circularNumber -docPath $record.'DOC_PATH' -isAdd $true;
...
Function AddAttachmentsFromFolder($ListItem, $circularNumber, $docPath, $isAdd) {
    ...
    try {
        ...
        #Get All List Item Attachment File
        ...
        if ($null -ne $AttachmentFiles) {
            $docFileName = $docPath.Split('\')[1];
            $docFile = $AttachmentFiles | Where-Object { ($_.FileName -eq $docFileName) }
            if ($null -ne $docFile) {
                $isAttachmentPresent = $true;
            }
        }

        if ($null -ne $docPath -and $isAttachmentPresent -eq $false) {
            ...
            if (Test-Path -Path $FullFilePath) {
                ...
                $global:logs += WriteLogs -circularNumber $circularNumber -isMigrated "Yes" -status "Attachment" -message "File Path Missing"; 
            }
        }
        else {
            if ($null -ne $docPath -and $isAttachmentPresent) {
